@group(0) @binding(0)
var<storage, read_write> data: array<vec2u, 148>;
@group(0) @binding(1)
var<storage, read_write> reduction: array<vec2u, 3>;
...
fn main(
  ...
  /*** begin load_multiple ***/
  {
    let base_striped_index = workgroup_id.x * 64u + local_id.x;
    {
      ...
      var lm_val: vec2u;
      if ( striped_index < ( ( ( 6209u ) + 63u ) / 64u ) ) {
  ...
  /*** begin (output write) ***/
  {
    ...
    if ( coalesced_data_index < ( ( ( 6209u ) + 63u ) / 64u ) ) {
