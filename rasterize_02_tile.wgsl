...
@group(0) @binding(1)
var<storage, read> initial_renderable_faces: array<TwoPassInitialRenderableFace>;
@group(0) @binding(2)
var<storage, read> initial_edges: array<LinearEdge>;
@group(0) @binding(3)
var<storage, read_write> coarse_renderable_faces: array<TwoPassCoarseRenderableFace>;
@group(0) @binding(4)
var<storage, read_write> coarse_edges: array<LinearEdge>;
@group(0) @binding(5)
var<storage, read_write> addresses: array<atomic<u32>>;
@group(0) @binding(6)
var<uniform> run_uniforms: u32;
struct LinearEdge {
  startPoint: vec2f,
  endPoint: vec2f
}
...
fn main(
  ...
  let skip_tile = source_face_index >= run_uniforms;
  ...
  coarse_renderable_faces[ face_index ] = TwoPassCoarseRenderableFace(
